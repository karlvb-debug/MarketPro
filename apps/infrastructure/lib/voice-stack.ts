...
export class VoiceStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: VoiceStackProps) {
    ...
    const pooledConnectInstance = new connect.CfnInstance(
      this,
      "MarketingSaaSConnectInstance",
      {
        attributes: {
          inboundCalls: false,
          outboundCalls: true,
    ...
    const outboundFlow = new connect.CfnContactFlow(
      this,
      "OutboundAmdContactFlow",
      {
        ...
        description:
          "Handles outbound campaign dialing, executes AMD, and plays poly voicemails.",
      },
    );

    // 4. Create SQS Queue for Voice dispatch with DLQ
    const voiceDispatchDlq = new sqs.Queue(this, "VoiceDispatchDLQ", {
    ...
    // 4.5. Provision Connect outbound campaign in Agentless mode
    const campaign = new connectcampaigns.CfnCampaign(
      this,
      "OutboundVoiceCampaign",
      {
        name: "MarketProOutboundVoiceCampaign",
        connectInstanceArn: pooledConnectInstance.attrArn,
        dialerConfig: {
        ...
        outboundCallConfig: {
          connectContactFlowArn: outboundFlow.attrContactFlowArn,
        },
      },
    );
    ...
    dispatchLambda.addToRolePolicy(
      new cdk.aws_iam.PolicyStatement({
        ...
        resources: [
          `arn:aws:connect:${this.region}:${this.account}:instance/${pooledConnectInstance.ref}/*`,
        ],
      }),
    );

    // Grant Connect Campaign sending permissions
    dispatchLambda.addToRolePolicy(
      new cdk.aws_iam.PolicyStatement({
        actions: [
        ...
        resources: [
          `arn:aws:connect-campaigns:${this.region}:${this.account}:campaign/${campaign.ref}`,
        ],
      }),
    );
    ...
    new cdk.CfnOutput(this, "VoiceDispatchQueueUrl", {
      value: this.voiceDispatchQueue.queueUrl,
    });
    new cdk.CfnOutput(this, "VoiceDispatchDlqUrl", {
      value: voiceDispatchDlq.queueUrl,
    });
    new cdk.CfnOutput(this, "ConnectCampaignId", {
      value: campaign.ref,
    });
  }
}
