...
// 3. Provision the API Stack (API Gateway, Authorizers)
...
// 4. Provision the Billing Stack (SNS Canonical, SQS, Stripe Webhooks, Idempotency)
...
// 5. Provision the Contact Ingestion Stack (S3, Step Functions)
...
// 6. Provision the Email Stack (SES Identities & IP Pools)
const emailStack = new EmailStack(app, 'MarketingSaaSEmailStack', { env });

// 7. Provision the SMS Stack (AWS End User Messaging, TCPA Compliance)
const smsStack = new SmsStack(app, 'MarketingSaaSSmsStack', { env });

// 8. Provision the Voice Stack (Amazon Connect Pooled Dialer, AMD flows)
const voiceStack = new VoiceStack(app, 'MarketingSaaSVoiceStack', { env });
